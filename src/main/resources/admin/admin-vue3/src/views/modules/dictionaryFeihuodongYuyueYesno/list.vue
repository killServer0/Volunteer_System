<template>
    <div class="main-content">
        ...
        <el-form :inline="true" :model="searchForm" class="form-content"
            ...
            <el-row :gutter="20" class="slt"
                ...
                <el-form-item :label="contents.inputTitle == 1 ? '报名状态' : ''">
                    <el-input v-model="searchForm.indexNameSearch" placeholder="报名状态" clearable
            ...
            <el-row class="ad"
                ...
                <el-form-item>
                    <el-button v-if="isAuth('dictionaryFeihuodongYuyueYesno', '新增')" type="success"
                    ...
                    <el-button v-if="isAuth('dictionaryFeihuodongYuyueYesno', '删除')"
        ...
        <el-table v-loading="dataListLoading" :data="dataList" border
            ...
            <el-table-column prop="codeIndex" label="报名状态编码" align="center" :sortable="contents.tableSortable"
                :align="contents.tableAlign" />
            <el-table-column prop="indexName" label="报名状态名称" align="center" :sortable="contents.tableSortable"
                :align="contents.tableAlign" />
            <el-table-column label="操作" width="300" align="center">
                <template #default="scope">
                    <el-button v-if="isAuth('dictionaryFeihuodongYuyueYesno', '查看')" type="success" size="small"
                    ...
                    <el-button v-if="isAuth('dictionaryFeihuodongYuyueYesno', '修改')" type="primary" size="small"
                    ...
                    <el-button v-if="isAuth('dictionaryFeihuodongYuyueYesno', '删除')" type="danger" size="small"
...
import { getDictionaryFeihuodongYuyueYesnoList, deleteDictionaryFeihuodongYuyueYesno } from '@/api/dictionaryFeihuodongYuyueYesno'
...
const getDataList = async () => {
    ...
    try {
        ...
        const { data } = await getDictionaryFeihuodongYuyueYesnoList(params)
        dataList.value = data.list
        totalPage.value = data.total
    } catch (error) {
        console.error('获取社区活动预约状态字典列表失败:', error)
        ElMessage.error('获取社区活动预约状态字典列表失败')
    } finally {
...
const deleteHandler = async (id) => {
    ...
    try {
        await ElMessageBox.confirm('确定要删除选中的报名状态吗？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        })
        await deleteDictionaryFeihuodongYuyueYesno(ids)
        ElMessage.success('删除成功')
        getDataList()
    } catch (error) {
        if (error !== 'cancel') {
            console.error('删除报名状态失败:', error)
            ElMessage.error('删除报名状态失败')
        }
    }
}
