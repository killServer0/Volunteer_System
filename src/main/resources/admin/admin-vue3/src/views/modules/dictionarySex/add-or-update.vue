<template>
    <div class="add-edit-block">
        <el-form ref="formRef" :model="form" :rules="rules" label-width="auto" class="detail-form-content"
            ...
            <el-row :gutter="20">
                <el-col :span="12">
                    <el-form-item v-if="type !== 'info'" label="性别类型编码" prop="codeIndex">
                        <el-input v-model="form.codeIndex" placeholder="性别类型编码" clearable :readonly="ro.codeIndex"
                    ...
                    <el-form-item v-else label="性别类型编码" prop="codeIndex">
                        <el-input v-model="form.codeIndex" placeholder="性别类型编码" readonly
                            :style="{ width: '100%', height: contents.inputHeight, lineHeight: contents.inputHeight, color: contents.inputFontColor, fontSize: contents.inputFontSize, borderWidth: contents.inputBorderWidth, borderStyle: contents.inputBorderStyle, borderColor: contents.inputBorderColor, borderRadius: contents.inputBorderRadius, backgroundColor: contents.inputBgColor }" />
                    </el-form-item>
                </el-col>
                <el-col :span="12">
                    <el-form-item v-if="type !== 'info'" label="性别类型名称" prop="indexName">
                        <el-input v-model="form.indexName" placeholder="性别类型名称" clearable :readonly="ro.indexName"
                            :style="{ width: '100%', height: contents.inputHeight, lineHeight: contents.inputHeight, color: contents.inputFontColor, fontSize: contents.inputFontSize, borderWidth: contents.inputBorderWidth, borderStyle: contents.inputBorderStyle, borderColor: contents.inputBorderColor, borderRadius: contents.inputBorderRadius, backgroundColor: contents.inputBgColor }" />
                    </el-form-item>
                    <el-form-item v-else label="性别类型名称" prop="indexName">
                        <el-input v-model="form.indexName" placeholder="性别类型名称" readonly
                            :style="{ width: '100%', height: contents.inputHeight, lineHeight: contents.inputHeight, color: contents.inputFontColor, fontSize: contents.inputFontSize, borderWidth: contents.inputBorderWidth, borderStyle: contents.inputBorderStyle, borderColor: contents.inputBorderColor, borderRadius: contents.inputBorderRadius, backgroundColor: contents.inputBgColor }" />
                    </el-form-item>
                </el-col>
            </el-row>
...
const rules = reactive({
    codeIndex: [
        { required: true, message: '性别类型编码不能为空', trigger: 'blur' },
        { pattern: /^\d+$/, message: '性别类型编码只能为数字', trigger: 'blur' }
    ],
    indexName: [
        { required: true, message: '性别类型名称不能为空', trigger: 'blur' }
    ]
})
...
const init = async (id, type) => {
    ...
    if (type === 'info' || type === 'update') {
        ...
        // 获取最大编码
        try {
            const { data } = await getMaxCodeIndex({ dicCode: 'sex_types' })
