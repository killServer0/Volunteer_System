    ...
    <div class="main-content">
        ...
        <el-form :inline="true" :model="searchForm" class="form-content"
            ...
            <el-row :gutter="20" class="slt"
                ...
                <el-form-item :label="contents.inputTitle == 1 ? '性别类型' : ''">
                    <el-input v-model="searchForm.indexNameSearch" placeholder="性别类型" clearable
            ...
            <el-row class="ad"
                ...
                <el-form-item>
                    <el-button v-if="isAuth('dictionarySex', '新增')" type="success" @click="addOrUpdateHandler()"
                    ...
                    <el-button v-if="isAuth('dictionarySex', '删除')" :disabled="dataListSelections.length <= 0"
        ...
        <el-table v-loading="dataListLoading" :data="dataList" border
            ...
            <el-table-column prop="codeIndex" label="性别类型编码" align="center" :sortable="contents.tableSortable"
                :align="contents.tableAlign" />
            <el-table-column prop="indexName" label="性别类型名称" align="center" :sortable="contents.tableSortable"
                :align="contents.tableAlign" />
            <el-table-column label="操作" width="300" align="center">
                <template #default="scope">
                    <el-button v-if="isAuth('dictionarySex', '查看')" type="success" size="small"
                    ...
                    <el-button v-if="isAuth('dictionarySex', '修改')" type="primary" size="small"
                    ...
                    <el-button v-if="isAuth('dictionarySex', '删除')" type="danger" size="small"
...
import { getDictionarySexList, deleteDictionarySex } from '@/api/dictionarySex'
...
const getDataList = async () => {
    ...
    try {
        ...
        const { data } = await getDictionarySexList(params)
        dataList.value = data.list
        totalPage.value = data.total
    } catch (error) {
        console.error('获取性别字典列表失败:', error)
        ElMessage.error('获取性别字典列表失败')
    } finally {
...
const deleteHandler = async (id) => {
    ...
    try {
        await ElMessageBox.confirm('确定要删除选中的性别类型吗？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        })
        await deleteDictionarySex(ids)
        ElMessage.success('删除成功')
        getDataList()
    } catch (error) {
        if (error !== 'cancel') {
            console.error('删除性别类型失败:', error)
            ElMessage.error('删除性别类型失败')
        }
    }
}
