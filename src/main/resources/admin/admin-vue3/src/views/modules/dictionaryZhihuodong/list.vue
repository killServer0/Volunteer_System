        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content"
                ...
                <el-row :gutter="20" class="slt"
                    ...
                    <el-form-item :label="contents.inputTitle == 1 ? '活动类型' : ''">
                        <el-input v-model="searchForm.indexNameSearch" placeholder="活动类型" clearable
                ...
                <el-row class="ad"
                    ...
                    <el-form-item>
                        <el-button v-if="isAuth('dictionaryZhihuodong', '新增')" type="success"
                        ...
                        <el-button v-if="isAuth('dictionaryZhihuodong', '删除')"
            ...
            <div class="table-content">
                <el-table :data="dataList" border v-loading="dataListLoading" @selection-change="selectionChangeHandler"
                    ...
                    <el-table-column prop="codeIndex" label="活动类型编码" align="center" :sortable="contents.tableSortable"
                        :align="contents.tableAlign" />
                    <el-table-column prop="indexName" label="活动类型名称" align="center" :sortable="contents.tableSortable"
                        :align="contents.tableAlign" />
                    <el-table-column label="操作" width="300" align="center">
                        <template #default="scope">
                            <el-button v-if="isAuth('dictionaryZhihuodong', '查看')" type="success" size="small"
                            ...
                            <el-button v-if="isAuth('dictionaryZhihuodong', '修改')" type="primary" size="small"
                            ...
                            <el-button v-if="isAuth('dictionaryZhihuodong', '删除')" type="danger" size="small"
                ...
                <el-pagination v-model:current-page="pageIndex" v-model:page-size="pageSize"
                    ...
                    @current-change="currentChangeHandle" class="pagination-content" :background="contents.pageBtnBG"
                    :style="{ textAlign: contents.pagePosition == 1 ? 'left' : contents.pagePosition == 2 ? 'center' : 'right' }">
                </el-pagination>
            </div>
        </div>
        <!-- 添加/修改页面 -->
        <add-or-update v-if="addOrUpdateFlag" ref="addOrUpdateRef" @refreshDataList="getDataList"></add-or-update>
    </div>
</template>
...
import { getDictionaryZhihuodongList, deleteDictionaryZhihuodong } from '@/api/dictionaryZhihuodong'
...
const getDataList = async () => {
    ...
    try {
        ...
        const { data } = await getDictionaryZhihuodongList(params)
        dataList.value = data.list
        totalPage.value = data.total
    } catch (error) {
        console.error('获取活动类型列表失败:', error)
        ElMessage.error('获取活动类型列表失败')
    } finally {
...
const deleteHandler = async (id) => {
    ...
    try {
        await ElMessageBox.confirm('确定要删除选中的活动类型吗？', '提示', {
            confirmButtonText: '确定',
            cancelButtonText: '取消',
            type: 'warning'
        })
        await deleteDictionaryZhihuodong(ids)
        ElMessage.success('删除成功')
        getDataList()
    } catch (error) {
        if (error !== 'cancel') {
            console.error('删除活动类型失败:', error)
            ElMessage.error('删除活动类型失败')
        }
    }
}
