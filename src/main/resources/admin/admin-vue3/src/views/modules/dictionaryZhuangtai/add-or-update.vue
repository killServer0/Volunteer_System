<template>
    <div class="add-edit-block">
        <el-form ref="formRef" :model="form" :rules="rules" label-width="auto" class="form-content"
            :style="{ backgroundColor: contents.formBgColor }">
            <el-row>
                <el-col :span="12">
                    <el-form-item v-if="type !== 'info'" label="状态编码" prop="codeIndex">
                        <el-input v-model="form.codeIndex" placeholder="状态编码" clearable :readonly="ro.codeIndex"
                            :style="{ backgroundColor: contents.inputBgColor, color: contents.inputFontColor, borderColor: contents.inputBorderColor }" />
                    </el-form-item>
            ...
            <el-form-item class="btn">
                <el-button v-if="type !== 'info'" type="primary" @click="onSubmit"
                    :style="{ backgroundColor: contents.btnSubmitBgColor, color: contents.btnSubmitFontColor, borderColor: contents.btnSubmitBorderColor }">提交</el-button>
                <el-button @click="back"
                    :style="{ backgroundColor: contents.btnCancelBgColor, color: contents.btnCancelFontColor, borderColor: contents.btnCancelBorderColor }">取消</el-button>
            </el-form-item>
        </el-form>
    </div>
</template>
...
import { getInfo, save, update, getMaxCodeIndex } from '@/api/dictionaryZhuangtai'
...
// 样式配置
const contents = ref(styleJs.addEditStyle())
...
const rules = reactive({
    codeIndex: [
        ...
        { pattern: /^[1-9]\d*$/, message: '状态编码只能为正整数', trigger: 'blur' }
    ],
    indexName: [
        { required: true, message: '状态名称不能为空', trigger: 'blur' },
        { min: 2, max: 20, message: '状态名称长度在2到20个字符之间', trigger: 'blur' }
    ]
})
...
const init = async (id, type) => {
    ...
    if (type === 'info' || type === 'update') {
        ...
        // 获取最大编码
        try {
            const { data } = await getMaxCodeIndex({ dicCode: 'zhuangtai_types' })
